...
>
> 状态更新（2026-04-04）：当前实际发布主线已收敛到 `dist/DecoScreenBeautifier.exe` + 可选内置 WT 资产；GUI 宿主源码继续保留，但不再默认打包为发布回退 EXE。
...
- [x] Windows Terminal 捆绑宿主已成为当前发布主线。
- [x] GUI 宿主（方案 B）当前仅保留源码与历史设计资产，不再作为默认打包入口。
