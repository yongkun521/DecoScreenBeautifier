...
- [x] 模板：GUI 中支持选择/应用模板（复用 `src/core/presets.py` 模板数据）
- [x] 设置：新增最小设置 UI（屏幕/置顶/效果开关/字体）
- [x] 缩放：`global_scale` 在 GUI 渲染中生效（cell size/字符密度/布局）
- [x] 持久化：窗口位置/尺寸/屏幕选择等在 `settings.json5` 持久化

### 0.3 V2（体验强化）

- [x] 后处理：像素级 CRT shader（先以 `QImage + NumPy` CPU 路线实现，包含曲率/扫描线/色散/暗角/噪点/slot mask）
- [x] 编辑器：迁移可视化编辑器到 GUI（已提供 GUI 弹窗编辑器，支持拖拽、网格吸附、尺寸调整与实时预览）
...
- [x] 新增 `src/gui_host/app.py`：Qt Application 启动与全局配置加载
- [x] 新增 `src/gui_host/window.py`：主窗口（无边框/置顶/多屏定位）
- [x] 新增 `src/gui_host/surface.py`：渲染表面（接收 RenderGrid 并绘制）
- [x] 新增 `src/gui_host/scene.py`：Scene 管理（组件、布局、合成、特效）
- [x] 新增 `src/gui_host/layout.py`：布局计算（layout config -> grid region mapping）
- [x] 新增 `src/gui_host/input.py`：快捷键与交互路由（d/e/t/q 等）
- [x] 新增 `src/gui_host/errors.py`：错误提示与回退策略（MessageBox、日志）
- [x] 新增 `src/gui_components/base.py`：GUI 组件接口（update/render）
- [x] 新增 `src/gui_components/hardware.py`：MVP 硬件组件（CPU/内存）
- [x] 新增 `src/gui_components/clock.py`：MVP 时钟组件（时间/日期）
- [x] 新增 `src/gui_components/network.py`：MVP 网络组件（上/下行）
...
- [x] 读取 `settings.json5`
- [x] 创建 Qt Application + 主窗口
- [x] 创建 Scene（加载模板/布局 + 初始化组件）
- [x] 启动渲染循环（QTimer）
- [x] 每帧执行渲染流程：
  - 组件采样（按各自频率）
  - 组件渲染到子网格
...
建议绘制策略：
- [x] 选择等宽字体，计算单元格像素大小（cell_w/cell_h）。
- [x] 对每一行做“同样式连续字符”合并（run-length encoding）：
...
- [x] 将窗口映射到“主字符网格”（例如 240x60）
- [x] 按 layout 的 row/col 把主网格划分为区域
- [x] 每个区域内渲染一个组件的子网格，再拷贝到主网格对应区域
...
阶段 1（最快落地）：
- [x] 新增“GUI 组件接口”，不依赖 Textual：
  - `update(dt)`（可选）
  - `render(width, height) -> RenderGrid`
- [x] 先实现 2~3 个核心组件（CPU/内存、时钟、网络）作为 GUI MVP
- [x] 现有 Textual 组件保留不动（legacy 模式继续用）

阶段 2（复用 Rich 渲染能力）：
- [x] GUI 组件内部可直接构造 Rich renderable（`rich.table.Table` / `rich.text.Text`）
- [x] 通过 `render_rich_to_grid(renderable, width, height)` 转成 RenderGrid
...
- [x] `auto`：优先非主屏中面积最大的屏幕
- [x] `primary` / `secondary` / index
- [x] `use_work_area`：使用工作区避免任务栏
...
兼容策略：
- [x] 如果 `gui_host.enabled` 不存在，默认关闭（保持旧行为）。
- [x] 发布阶段可将 GUI 作为默认入口 EXE（用户无需配置）。
...
- [x] 新增 `src/main_gui.py`：GUI 默认入口（最终发布 EXE 使用）
- [x] 保留 `src/main.py`：现有 Textual 入口（开发/应急）
- [x] 依赖检测：GUI 模式下复用/改写依赖检测逻辑（避免仅靠 stderr 提示）
...
- [x] 自动验收脚本：新增 `scripts/validate_gui_host.py`，输出 `report.json` + `metrics.json` + `gui_perf.jsonl`
- [x] 本机复测（2026-02-07）：`scripts/validate_gui_host.py` 已 PASS，`report.json` 全检查项通过

### 10.2 性能验收

- [~] 性能目标：1080x480 / 1920x480 场景下 CPU 占用可控（临时阈值：平均 FPS >= 20、平均 CPU <= 85%）
- [~] 性能统计：新增 `scripts/benchmark_gui_host.py`，按 1080x480 / 1920x480 自动跑分并输出报告
- [~] 本机复测（2026-02-07）：Qt 运行时已可用，性能样本可产出，但当前结果未达阈值（1080x480: 12.31 FPS / 98.36% CPU；1920x480: 7.32 FPS / 99.37% CPU）

## 11. 里程碑拆解（待办）

### 11.1 MVP（可运行的 GUI 宿主）

- [x] 新增 `src/gui_host/*`：Qt 主窗口 + 渲染表面 + Scene/布局
- [x] 渲染实现：`RenderGrid -> QPainter`（行段合并 + 背景填充策略）
- [x] 合成实现：layout -> subgrid -> main grid + 全局 effects 管线
- [x] MVP 组件：clock/hardware/network（至少 2~3 个）
- [~] 打包产物：GUI EXE（无控制台）+ legacy EXE（保留终端入口）
- [~] 通过验收：完成“10.1/10.2”所有勾选项（已形成可复现闭环，待 Qt 运行时环境复测通过）

### 11.2 V1（可用性增强）

- [x] 模板：支持选择/应用模板（复用现有模板数据）
- [x] 设置：最小设置界面（开关/屏幕/置顶/效果/字体）
- [x] 缩放：`global_scale` 全面接入 GUI 渲染与布局

### 11.3 V2（体验强化）

- [x] 后处理：像素级 CRT shader（色散/模糊/曲率/抖动等）
- [x] 编辑器：迁移布局编辑器到 GUI（拖拽/对齐/预览）
