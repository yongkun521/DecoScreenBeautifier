...
- [x] **编辑器界面 (Editor UI)**
    ...
    - [x] 修复 `Open Editor` 闪退（避免自定义 `_set_dirty` 与 Textual 内部刷新方法冲突，并修复编辑器状态提示的主题属性访问错误）
    - [x] 调整编辑器响应式断点，避免中等窗口宽度下误切为整页纵向堆叠，导致只显示首个面板
...
## 2026-04-06 图片组件体验优化
- [x] 图片组件新增三种显示模式：
  - [x] `Scale to Fit`：等比缩放，完整显示图片内容。
  - [x] `Fill and Crop`：等比填充，必要时自动居中裁切。
  - [x] `Stretch`：保留原有强制拉伸模式，兼容旧习惯。
- [x] 图片组件布局数据新增 `image_display_mode` 字段，主显示页已按布局真实生效。
- [x] Textual 编辑器图片属性面板新增 `Image Display` 选择器。
- [x] Textual 编辑器图片路径输入增强：
  - [x] 新增 `Browse...` 按钮，调用系统文件选择器选择图片。
  - [x] 新增 `Paste Path` 按钮，从系统剪贴板读取路径。
  - [x] 新增 `Ctrl+V` 优先走系统剪贴板的粘贴处理，减少终端宿主差异带来的输入问题。

## 2026-04-06 副界面响应式与保存语义优化
- [x] `Layout Editor` 响应式布局增强：
  - [x] 工具箱 / 画布 / 属性区改为“内容区自适应 + 操作区固定可见”。
  - [x] 小窗口下自动切换为紧凑排版，避免按钮被挤出可视区域。
- [x] `Template Selector` 响应式布局增强：
  - [x] 模板筛选区与操作区在小窗口下自动改为更紧凑的纵向排版。
  - [x] 模板操作按钮保持在页面下方固定可达。
- [x] `Layout Editor` 交互语义调整：
