...
- [ ] **模板扩展与尺寸适配**
    ...
    - [~] 模板切换自动应用布局 + 主题样式 + 字体预设 + 全局缩放参数
    - [ ] 模板列表支持标签/筛选（按屏幕比例、用途、风格）
- [ ] **组件样式与布局变体**
    - [~] 为核心组件提供 2-3 套样式变体（密集/极简/仪表盘/横条版）
    - [~] 为长条屏增加紧凑与纵向布局变体（组件标题、边框、内边距自适应）
    - [ ] 统一样式 Token（边框、条形、标题、颜色）供模板复用
- [ ] **字体/字符预设体系**
    - [x] 建立字体预设注册表（标题字体、条形字符、趋势线字符、图像字符集）
    - [x] 提供多套字符集/字形风格（细线/块状/点阵/故障风格）
    - [~] 模板默认绑定字体预设，可在设置中手动覆盖
- [ ] **全局缩放参数**
